...
class PromptNodeProcessor:
    ...
    def process(self, replay, node: str) -> None:
        """
        ...
        Args:
            ...
            node: The node identifier to process
        """
        logger.info(f"Processing PROMPT node {node}")
        
        try:
            # Extract node data and build request
            node_data = self._extract_node_data(replay, node)
            llm_request = self._build_llm_request(node_data, replay)
            
            # Send request to LLM
            response_data = self._send_llm_request(replay, llm_request)
            
            # Process and save response
            self._process_llm_response(response_data, replay)
    ...
    def _extract_node_data(self, replay, node: str) -> Dict[str, Any]:
    ...
    def _build_llm_request(self, node_data: Dict[str, Any], replay) -> LLMRequest:
    ...
    def _load_read_only_files(self, docs_refs: List[str], template_refs: List[str], base_dir: str) -> List[FileReference]:
    ...
    def _load_files_from_directory(self, file_refs: List[str], base_dir: str, file_type: str) -> List[FileReference]:
    ...
    def _process_llm_response(self, response_data: Dict[str, Any], replay) -> None:
