...
class RunNodeProcessor:
    def process(self, replay, node):
        ...
        command_to_run = replay.epic.graph.nodes[node]['contents']['command_to_run']
        ...
        with open(os.path.join(replay.replay_dir, "run_tests_pass_fail.txt"), "r") as f:
            replay.epic.graph.nodes[node]['contents']['passed'] = f.read()
        logger.debug(f"Test results: {replay.epic.graph.nodes[node]['contents']['passed']}") 